...
module.exports = {
    ...
    async execute(interaction: CommandInteraction) {
        ...
        try {
            SettingsHandler(interaction.options, interaction.guild?.id as string, "leave");

            await interaction.reply({ content: "Leave message set!", ephemeral: true });
        } catch (err) {
            console.error(err);
            return await interaction.reply({ content: "An error occurred, while setting the message.", ephemeral: true });
        }
    }
}